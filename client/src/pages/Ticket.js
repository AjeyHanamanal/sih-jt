...
const Ticket = () => {
  ...
  const qrPayload = encodeURIComponent(JSON.stringify({ status, dest, amount, pid, bookingId, ts: Date.now() }));
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex items-center justify-center px-4">
      <div className="card p-6 w-full max-w-md text-center">
        <h1 className="text-2xl font-semibold mb-2">{status === 'success' ? 'Scan the below QR to pay' : 'Payment'}</h1>
        ...
        <div className="text-left text-sm bg-gray-50 border rounded p-4 space-y-1">
          {bookingId && <div><span className="text-gray-500">Booking ID:</span> {bookingId}</div>}
        ...
        <div className="mt-6 flex gap-3 justify-center">
          ...
          {bookingId && <Link to="/tourist/bookings" className="btn-secondary">View My Bookings</Link>}
        </div>
